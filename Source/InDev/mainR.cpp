...
void OnUpdate(File* f) {
    ComponentFile::fileUpdated(f);
    for (auto i : Object::objects) {
        for (auto j : i->components) {
            j.comp->Start();             // следить за временными компонентами
        }
    }
}
...
int main()
{
    Path::SetPath("C:/Users/yaffr/source/SimpleEngine/");

    //VSProject::VCreateProject("Project24");
    VSProject::VSelectProject("Project24");

    //VSProject::VCreateFile("AActor");
    //VSProject::VCreateFile("APawn");
    ComponentFile cft("AActor");
    ComponentFile cff("APawn");

    //VSProject::VOpenProject();
    //VSProject::VOpenFile("Killer");
    //VSProject::VOpenFile("Grabber");

    
    FileWatcher::onUpdate(OnUpdate);
    FileWatcher::Start();
    //
    //
    Object::Create("obj1");
    ...
    //
    //
    //Object::findObject("obj1").components[0].comp->Start();
    //Object::findObject("obj2").components[0].comp->Start();
...
(для продолжения нажмите клавишу ВВОД)
  ...
  templateScope[-]      применяет стандартные правила удаленного управления параметрами шаблона C++
  enumTypes[-]          включить базовые типы перечислений C++ Стандартный (по умолчанию отключены)
  checkGwOdr[-]         применять стандартные нарушения правила одного определения C++,
                        когда включен параметр /Gw (по умолчанию отключено)
